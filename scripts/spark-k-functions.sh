...
# (spark_k_setup) Generates node-list files. It determines the nodes
# for a master and workers, and generates a host-list file and a
# setting file.  It is called by the main job script (a single
# process).  A host-list file consists of a list of workers.  A
# setting file contains variables "${k_master_node}" and
# "${k_master_url}".  It assigns a single master and (nprocs-1)
# workers.  Optional 1st argument limits the number of nodes.  Note
# that the files are created in "${PJM_JOBDIR}", which points to a
# so-called "shared"-directory (it is one up from a "rank"-directory)
# and accessible from all ranks.
...
# workers processes.  It reads files generated by "spark_k_setup()".
# It waits until all processes are certainly started.  It is called by
